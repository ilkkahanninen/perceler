 ...
 * engine switches to an offline time base: a virtual clock advances by
 * exactly 1000/60 ms per frame, audio is rendered one frame's worth
 * per iteration via libxmp (not the hardware drivers), and both video
 * and audio are streamed to disk.
 *
 * Output:
 *   <prefix>.RAW  — video. Magic "PRCL" then, per frame:
 *                     768   VGA DAC palette (R,G,B, values 0-63)
 *                     64000 pixel indices (320 × 200)
 *   <prefix>.WAV  — standard PCM WAV. Stereo 16-bit signed @ 22050 Hz.
 *                   RIFF and data chunk sizes are patched on close.
 *
 * Post-process with tools/capture2png.py (video) and ffmpeg (assembly).
 */
