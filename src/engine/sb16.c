...
/* Double-buffer state                                                  */
/* ------------------------------------------------------------------ */
#define HALF_BYTES ((unsigned long)(SB16_HALF_SAMPLES) * 2 * 2) /* stereo*16bit */
...
static int dsp_reset(void)
{
    int i;
    outp(SB_RESET, 1);
    for (i = 0; i < 100; i++) inp(0x80); /* ~3 µs via port-80 delay */
    outp(SB_RESET, 0);
    for (i = 0; i < 65536; i++)
...
static void parse_blaster(void)
{
    ...
    while (*p) {
        char c = *p++;
        if      (c == 'A' || c == 'a') sb_base = (unsigned short)strtol(p, &p, 16);
        else if (c == 'I' || c == 'i') sb_irq  = (unsigned char)strtol(p, &p, 10);
        else if (c == 'H' || c == 'h') sb_hdma = (unsigned char)strtol(p, &p, 10);
        else while (*p && *p != ' ') p++;
        while (*p == ' ') p++;
    }
...
int sb16_init(sb16_fill_fn fill)
{
    ...
    int386(0x31, &r, &r);
    if (r.x.cflag) return SB16_ERR_MEMORY;
    ...
    /* --- Reset DSP --- */
    if (dsp_reset() != 0) {
        r.w.ax = 0x0101; r.w.dx = g_dsel; int386(0x31, &r, &r);
        return SB16_ERR_DSP;
    }
    ...
    r.w.ax = 0x0204; r.h.bl = g_irq_vec;
    int386(0x31, &r, &r);
    g_old_sel = r.w.cx;
    g_old_off = r.x.edx;

    segread(&sr);
    r.w.ax  = 0x0205; r.h.bl = g_irq_vec;
    r.w.cx  = sr.cs;
...
void sb16_update(void)
{
    ...
    if (g_fill) g_fill(ptr, SB16_HALF_SAMPLES);
    g_need = 0;
}
